...
Malaysia <- global_data[global_data$Country == keyword, ]

#check for missing values
#specify columns 5 to 8 to check for missing values
if (any(is.na(Malaysia[, 5:8]))) {
...
cases <- Malaysia[Malaysia$New_cases != 0, ]
...
first_date <- which(Malaysia == first_line, arr.ind=TRUE)[1,1]

# find the number of row of the last date in the Malaysia data frame
last_date <- which(Malaysia == "2022-12-31", arr.ind=TRUE)[1,1]

# create a new data frame from first confirmed date until 31st dec 2022
confirmed_cases <- Malaysia[first_date:last_date, ]

confirmed_cases <- confirmed_cases[,-2:-4]
# write to csv
write.csv(confirmed_cases, "/Users/chairunnisanuramanda/RAMNDA/covid_malaysia.csv", row.names=TRUE)
